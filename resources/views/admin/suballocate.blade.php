...
<!--grid-->
 	...
 	<!---->
<!-- Div form-->
    <form action="{{ action('FacaltyAllocationController@store') }}" method="post">
		{{ csrf_field() }}
		<div class="clearfix"> </div>
        <div class="col-md-12 form-group2 group-mail">
              <label class="control-label">Select Teacher</label>
            <select name="teacher" id="teacher">
				<option disable>select Teacher</option>
				@foreach($teacherdata as $tdata)
					<option value="{{ $tdata->id }}" >{{ $tdata->fname }} {{ $tdata->lname}}</option>
				 @endforeach
            </select>
            </div>
              <div class="col-md-12 form-group2 group-mail">
            ...
            <select name="class" id="class">
				<option disable>select Course</option>
        ...
        <div class="col-md-12 form-group2 group-mail">
			<label class="control-label">Select Div</label>
			<select name="div_id" id="div_id"></select>
		</div>
        <div class="col-md-12 form-group2 group-mail">
				...
				$.ajax({
					...
					success:function(data) {
						...
						if(data){
							...
							$.each(data, function(key, value){
                        		$('select[name="subject_id"]').append('<option value="'+ value.id +'">' + value.subject+ '</option>');
							});
						}else{
                    		$('#subject_id').empty();
                  		}
